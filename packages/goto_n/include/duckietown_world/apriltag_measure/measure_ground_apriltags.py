...
class Apriltag_measurer():
    ...
    def get_ground_tag_dict(self):
        ...
        if not self.map_yaml.get("objects"):
    ...
    def update_apriltags(self):
        ...
        while True:
            ...
            if at_number in self.ground_tag_dict:
                ...
                at_dict = self.get_at_dict(at_number)
                new_name = "tag%i" % at_number
                if not result_map_yaml.get("objects"):
                    result_map_yaml["objects"] = dict()
                result_map_yaml["objects"][new_name] = at_dict
                self.ground_tag_dict[at_number] = new_name
...
def uploadFromCSV(map_file, csv_file):
    map_yaml = load_yaml_file(map_file)
    x_offset = 0.012
    y_offset = 0.012
    tile_size = float(map_yaml["tile_size"])
    with open(csv_file) as f:
        # r = csv.reader(codecs.EncodedFile(f, 'utf-8', 'utf-8-sig'))
        r = csv.reader(f)
        for row in r:
            tag_id_str = ""
            for c in row[0]:
            ...
            x_count, y_count = int(row[1]), int(row[2])
            x_measure, y_measure = float(row[3]), float(row[4])
            angle_measure = int(row[5])
            complete_x = tile_size * x_count + x_measure + x_offset
            complete_y = tile_size * y_count + y_measure + y_offset
            apriltag = Apriltag(tag_id, complete_x, complete_y, angle_measure)
            tag_name = "tag%i" % tag_id
            if not map_yaml.get("objects"):
                map_yaml["objects"] = dict()
            map_yaml["objects"][tag_name] = apriltag.to_dict()
...
def main():
    ...
    parser.add_argument('-t', '--tags_csv', type=str,
                        help='path to csv file storing tag info')
    ...
    if csv_file == None:
        ...
        if not os.path.isfile(csv_file):
            error("The file %s does not exist" % csv_file)
        else:
            result_map_yaml = uploadFromCSV(full_path, csv_file)
